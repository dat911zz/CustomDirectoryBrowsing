...
namespace DirectoryListing.Models
{
    public class Directory
    {
        public Directory(string rootDirectory, string fullPath)
        {
            ...
            Current = new Info(rootDirectory, directoryInfo.FullName, directoryInfo.Name, 0, directoryInfo.CreationTime);
            Directories =
                directoryInfo
                    .EnumerateDirectories()
                    .Where(x => !x.Attributes.HasFlag(FileAttributes.Hidden))
                    .OrderByNatural(x => x.Name)
                    .Select(x => new Info(rootDirectory, x.FullName, x.Name, 0, x.CreationTime))
                    .ToArray()
                ;
            Files =
                directoryInfo
                    ...
                    .Select(x => new Info(rootDirectory, x.FullName, x.Name, x.Length, x.CreationTime))
                    .ToArray()
                ;
